...
import { ChevronDoubleDownIcon, ChevronDoubleUpIcon, EqualsIcon } from '@heroicons/react/20/solid'
    ...
    >
        ...
        <div className="flex gap-4 justify-between py-2 text-gray-500 text-sm">
            <div className="flex gap-2">
                <div>{task.id}</div>
                {task.priority === 'high' && <ChevronDoubleUpIcon />}
                {task.priority === 'medium' && <EqualsIcon />}
                {task.priority === 'low' && <ChevronDoubleDownIcon />}
            </div>
